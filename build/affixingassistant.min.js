...
centerViewAtNode(selector){let $node=$(selector);if($node.length<=0)return;let $container=$("#mastercontainer");if($container.has($node).length<=0)return;let zoomScale=$container.panzoom('getMatrix')[0];let toNodeCenter={left:($node.outerWidth()/2)+(Math.round($node.offset().left-$container.offset().left)/zoomScale),top:($node.outerHeight()/2)+(Math.round($node.offset().top-$container.offset().top)/zoomScale)}
