...
${CHECKBOX_TEMPLATE({label:`Use All Six Fodders`,description:`Makes it cheaper to produce the intermediary equipment`,isChecked:shouldSpread})}</div>` 
            : ``}<div class="content">`;if(isAffixSelection){layoutTemplate+=`<div><div>
                        <div class="title bold">Affixing Goal</div>
                        <div class="selection-container">`;for(var i=0;i<(new Fodder).CAPACITY;i++){layoutTemplate+=`<div class="affix${(affixesSelected[i]) ? `` : ` empty`}"${(affixesSelected[i]) ? ` title="${affixesSelected[i].effect}"` : ``}${(affixesSelected[i]) ? ` data-code="${affixesSelected[i].code}"` : ``}>
...
setActiveFodder(e){let vc=(this instanceof ViewController)?this:(e.data&&e.data.viewcontroller)?e.data.viewcontroller:undefined;if(vc.isPanning)return;if(!(vc instanceof ViewController))return;let found=vc.findFodderAndNodeByDOM(this);vc.assistant.setActiveFodder(found.fodder);vc.assistant.setActivePageTreeNode(found.pageTreeNode);vc.affixesSelected=found.fodder.affixes.slice(0);if(vc.assistant.pageTreeRoot==null||vc.assistant.pageTreeRoot==found.fodder){stringData.affixingSrc='Reproducing Goal';try{gaRequests.send('main','buttonClick',{'View Type':'Main View','Interface Type':'Reproduce Button','Number Of Interfaces Used':1})}
catch(e){}}
...
findFodderAndNodeByDOM(fodderElem){let $elem=$(fodderElem);if(!$elem.hasClass('fodder'))$elem=$elem.parents('div.fodder');if($elem.length<=0||!(this.assistant.pageTreeRoot instanceof PageTreeNode))return;let indices=[];do{if($elem.length>0)indices.unshift($elem.index());$elem=$elem.parents('table.mgrid')}while($elem.length>0);indices.splice(0,1);let curr=this.assistant.pageTreeRoot;for(var i=0;i<indices.length-1;i++){curr=curr.children[indices[i]]}
...
$curr=$($curr.find('div.page')[0]);else if(trackingRoute[i]instanceof PageTreeNode&&$curr.length>0){let pageTreeNode=trackingRoute[i];let next=trackingRoute[i+1];if(next&&next instanceof PageTreeNode){let indexOf=pageTreeNode.children.indexOf(next);$curr=$($($curr[0]).find('td')[1]).children('.mgrid');$curr=$($curr[indexOf])}}}
return $curr}
...
else $('div.link-container').removeClass('hidden');$('div.link-container div.copy-button:nth-child(3)').click(()=>{let copyText=$('div.link-container input[type=text]')[0];if(!copyText)return;copyText.select();document.execCommand("copy");try{gaRequests.send('share','buttonClick',{'View Type':'Share Link View','Link Type':'Link to Assistant','Number Of Links Used':1})}
...
urlParams=urlParams.substring(1,urlParams.length);urlParams=decodeURIComponent(urlParams);urlParams=VIEW_CONTROLLER.assistant.decodeURLParams(urlParams);if(!this.assistant||!(this.assistant instanceof Assistant))return;let hasSuceeded=this.assistant.loadFromURLParams(urlParams);if(hasSuceeded){let $container=$('#mastercontainer');let dimsBefore={width:$container.outerWidth(),height:$container.outerHeight()}
this.updateView();let newpos={left:$container.panzoom('getMatrix')[4]*$container.outerWidth()/dimsBefore.width,top:$container.panzoom('getMatrix')[5]*$container.outerHeight()/dimsBefore.height}
$container.panzoom('pan',newpos.left,newpos.top,{animate:!1,});this.isPanning=!1}}
updateView(e){let vc=(this instanceof ViewController)?this:(e.data)?e.data.viewcontroller:undefined;if(!(vc instanceof ViewController))return;if(!vc.assistant||!vc.assistant.pageTreeRoot||!(vc.assistant.pageTreeRoot instanceof PageTreeNode))return;vc.assistant.calcSuccessRates();$('#mastercontainer').empty().append(PAGE_TREE_NODE_TEMPLATE({pageTreeNode:vc.assistant.pageTreeRoot}));vc.regenerateConnections();$('div.fodder').hover(spotlightIn,spotlightOut);$('div.produce-button:not(.disabled)').click({viewcontroller:this},this.setActiveFodder).on('mousedown touchstart',function(e){e.stopImmediatePropagation()});$('.boost-container input[type=checkbox]').change({viewcontroller:this},(e)=>{if(e.data.viewcontroller.isPanning)return;let $elem=$(e.currentTarget);for(var i=0;i<$elem.length;i++){let{fodder}=e.data.viewcontroller.findPageAndNodeByDOM($elem[i]);if(fodder)fodder.setSameGear($elem.prop('checked'))}
...
catch(e){}});$('.checkmark').click((e)=>{if(this.isPanning)e.preventDefault()});$('.checkbox-container').on('mousedown touchstart',function(e){e.stopImmediatePropagation()});$('.boost-container > div.dropdown-container:not(:last-child) select').change({viewcontroller:this},(e)=>{let $elem=$(e.currentTarget);for(var i=0;i<$elem.length;i++){let index=$($elem[i]).find('option:selected').index();let{fodder}=e.data.viewcontroller.findPageAndNodeByDOM($elem[i]);if(fodder)fodder.setRateBoostIdx(index)}
...
catch(e){}}).on('pointerdown mousedown touchstart',function(e){e.stopImmediatePropagation()});$('.boost-container > div.dropdown-container:last-child select').change({viewcontroller:this},(e)=>{let $elem=$(e.currentTarget);for(var i=0;i<$elem.length;i++){let index=$($elem[i]).find('option:selected').index();let{fodder}=e.data.viewcontroller.findPageAndNodeByDOM($elem[i]);if(fodder)fodder.setPotentialIdx(index)}
...
catch(e){}}).on('pointerdown mousedown touchstart',function(e){e.stopImmediatePropagation()});vc.displayWelcomeScreen(!1);return this}
...
centerViewAtNode(selector){let $node=$(selector);if($node.length<=0)return;let $container=$("#mastercontainer");if($container.has($node).length<=0)return;let pos={left:0,top:0};let $curr=$node;while($curr.attr('id')!='mastercontainer'){let currPos=$curr.position();pos.left+=currPos.left;pos.top+=currPos.top;$curr=$curr.parent()}
let zoomScale=$container.panzoom('getMatrix')[0];let toNodeCenter={left:($node.outerWidth()/2)+(Math.round(pos.left)/zoomScale),top:($node.outerHeight()/2)+(Math.round(pos.top)/zoomScale)}
...
$container.panzoom('pan',newpos.left,newpos.top,{animate:!1,});this.isPanning=!1;return this}
...
return results;function isRefAMatch(ref){let isMatch=!0;if(dataClass){if(ref instanceof dataClass){if(properties){for(var key in properties){if(typeof properties[key]!=='object'){if(ref[key]!=properties[key]){isMatch=!1;break}}
